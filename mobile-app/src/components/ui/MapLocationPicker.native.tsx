...
async function reverseGeocode(lat: number, lon: number): Promise<GeoResult> {
  try {
    ...
    const res = await fetch(url, {
      headers: { "Accept-Language": "en", "User-Agent": "TripGenieApp/1.0" },
    });
...
export function MapLocationPicker({
  ...
  const handleZoom = useCallback((direction: "in" | "out") => {
    ...
    const next: Region = {
      ...
      latitudeDelta: Math.min(Math.max(currentRegion.current.latitudeDelta * factor, 0.002), 120),
      longitudeDelta: Math.min(Math.max(currentRegion.current.longitudeDelta * factor, 0.002), 120),
    };
    ...
    <Modal visible={visible} animationType="slide" statusBarTranslucent onRequestClose={onClose}>
      <View style={styles.root}>
        ...
        <View style={styles.zoomCluster} pointerEvents="box-none">
          <Pressable style={styles.zoomBtn} onPress={() => handleZoom("in")} hitSlop={6}>
            <Ionicons name="add" size={22} color={colors.ink} />
          </Pressable>
          <View style={styles.zoomDivider} />
          <Pressable style={styles.zoomBtn} onPress={() => handleZoom("out")} hitSlop={6}>
            <Ionicons name="remove" size={22} color={colors.ink} />
          </Pressable>
        </View>

        {/* ── Top bar ── */}
        <SafeAreaView style={[styles.topBar, Platform.OS === "android" && styles.topBarAndroid]}>
...
function makeStyles(c: Colors) {
  return StyleSheet.create({
    ...
    zoomDivider: { height: 1, backgroundColor: c.line, marginHorizontal: 10 },
    topBar: {
      position: "absolute",
      top: 0,
      left: 0,
